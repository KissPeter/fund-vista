...
- ``methods`` owns the ``contour | hatch | flow`` strategies behind one protocol,
  so adding a new method means adding one class + one registry line.
- ``optimize`` owns the vpype-equivalent cleanup (linemerge / simplify / sort /
  reloop / layout) in pure Python — no GPL binaries required (see README note
